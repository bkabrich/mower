...
/** Minimum inset (degrees) so path stays clearly inside even with polyline stroke. */
const MIN_INSET = 0.00015
/** Fraction of boundary size to inset (e.g. 0.08 = 8%). */
...
/** Return inset bounds so path is strictly inside the polygon (no stroke spill). */
...
/** Clamp every point to the inset bounds so the polyline never crosses the boundary. */
...
/** Generate parallel stripe path inside boundary (axis-aligned fallback). */
...
/** Clamp an existing path to stay inside a boundary (e.g. for display). */
